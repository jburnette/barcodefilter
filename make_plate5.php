...
while ($plate_fetch = $select_plates_stmt->fetch()) {
	...
	while ($col_num < "I") { 
		...
		while ($row_letter < "13") {
			...
			$table .= "<a href=" .'./get_fasta5.php?plate=' .$plate. '&type=A'. '&forward=' .$fp. '&reverse='. $rp.  '&db='. $database ." target='_blank'>&nbsp" .'All'. "</a>&nbsp";	
			$table .= "<a href=" .'./get_fasta5.php?plate=' .$plate. '&type=C'. '&forward=' .$fp. '&reverse='. $rp.  '&db='. $database ." target='_blank'>&nbsp" .'Collapsed'. "</a> </td>";	
