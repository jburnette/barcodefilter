...
**/
$db = new PDO('sqlite:../Databases/salsa_2018_2.1.db');
$select = "SELECT * FROM seqs WHERE id = 59";
...
$odd_insert = "INSERT INTO odds (primer, seq, name, which_odd, location) VALUES (:primer, :seq, :name, :which, :location)";

try {
	$odd_insert_stm = $db->prepare($odd_insert);
} catch (PDOException $ex) {die($ex);}
/** New strategy
...
while ($seq = $stmt->fetch()) {
	...
	//print_r($positions);
		...
		split_read($seq, $positions, $insert_stm, $primer_select_stm, $odd_insert_stm);
		$count++;
		echo ($count . "\n");
