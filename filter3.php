...
/** Searches for primers
	...
	3. push into array.
**/
$out_file = fopen("no_primer.fasta", "w");
$count = 0;
$no_count = 0;
$found = 0;
$r_found = 0;
...
while ($read = $stmt->fetch()) {
	...
	foreach ($r_primers as $r_seq) {
		$result = strpos($read["sequence"], $r_seq[2]) || strpos($read["sequence"], $r_seq[3]);
		...
		if ($result !== false) {
			...
			foreach ($f_primers as $f_seq) {
				$f_result = strpos($read["sequence"], $f_seq[2]) || strpos($read["sequence"], $f_seq[3]);
				if ($result !== false) {
					$read_name = $read['name'];
					$found++;
			...
			//array_push ($no_primer, $read["name"]);
			$line_out = $read["name"]."\n".$read["sequence"] ."\n";
			//echo $line_out;
			fwrite($out_file, $line_out);
			$no_count++;
			echo $no_count . "\n";
			break;
